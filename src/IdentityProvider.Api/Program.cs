...
builder.Services.AddSingleton<IPasswordHasher<UserIdentity>, PasswordHasher<UserIdentity>>(); // Default: PBKDF2. Consider BCrypt or Argon2 in production.
builder.Services.AddSingleton<IAuthService, AuthService>();
builder.Services.AddSingleton<ITokenService, JwtTokenService>();
builder.Services.Configure<JwtSettings>(builder.Configuration.GetSection("JwtSettings"));
builder.Services.AddSingleton<IUserRepository, InMemoryUserRepository>();
builder.Services.AddSingleton<IdentityContext>(sp =>
{
...
builder.Services.AddCors(options => options.AddDefaultPolicy(policy =>
{
    ...
    policy.WithOrigins("https://localhost:7034").WithMethods("GET", "POST").AllowAnyHeader();
}));
